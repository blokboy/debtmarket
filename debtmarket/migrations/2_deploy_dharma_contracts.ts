...
const AssetCollateralizer = artifacts.require("AssetCollateralizer");
const CDO = artifacts.require("CDO");

module.exports = (deployer: any, network: string, accounts: string[]) => {
    const TX_DEFAULTS = { from: accounts[0], gas: 40000000 };
    ...
    deployer.deploy(DebtRegistry).then(async () => {
        ...
        await deployer.deploy(AssetCollateralizer);
        await deployer.deploy(CDO);
    });
};
